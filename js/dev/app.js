...
function updatePhysics() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (distanceSquared < radiusSquared && distanceSquared > 0) {
      ...
      const distFromOrigin2 = Math.sqrt(
        (currentX - particle.originalX) ** 2 + (currentY - particle.originalY) ** 2
      );
      const forceMultiplier = Math.max(
        0.1,
        1 - distFromOrigin2 / (config.maxDisplacement * 2)
      );
    ...
    const targetX = currentX + particle.velocityX + (particle.originalX - currentX) * config.returnForce;
    const targetY = currentY + particle.velocityY + (particle.originalY - currentY) * config.returnForce;
  ...
  window.addEventListener("resize", () => {
    ...
    if (particles.length > 0) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const col = i % dim;
        const repositionX = centerX + (col - dim / 2) * 1;
        const repositionY = centerY + (row - dim / 2) * 1;
